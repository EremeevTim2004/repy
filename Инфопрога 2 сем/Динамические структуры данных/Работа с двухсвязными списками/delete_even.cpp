...
int main() {
    int *a = new int[10]{3, 3, 1, 4, 1, 7, 9, 2, 8, 1};
    list *h, *t, **ll;
    ll = make(a, 10); // Создаём новый двусвязный список
    ...
    while (curr != NULL) {
        ...
        if (curr->inf % 2 == 0) del_node(h, t, curr); // Если значение чётное, то удаляем
        curr = next;                           // Переходим на следующий
    }
    print(h, t);                               // Печатаем
    cout << endl;
    return 0;
