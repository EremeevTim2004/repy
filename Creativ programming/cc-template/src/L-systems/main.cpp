...
struct Turtle {
    ...
    Turtle(Vector2 pos, float angle, float thickness, Color color) : pos(pos), angle(angle * DEG2RAD), thickness(thickness), color(color) {}
...
int main()
{
    ...
    std::map<char, std::string> rules = {{'F', "FF-[-F+F+F]+[+F-F-F]"}, {'+', "+"}, {'-', "-"}, {']', "]"}, {'[', "["},};
        ...
        BeginDrawing();
        {
            ...
            for (int j = 0; j < axiom.length(); ++j)
            {
                switch (axiom[j])
                {
                    case 'F':
                    {
                        turtle.color = Color {coloring(j, 1, 0), coloring(j, 1, 120), coloring(j, 1, 240), 255};
