...
v2d screen_to_local(context_t *ctx, v2d point)
{
    v2d d = {ctx->viewport.right - ctx->viewport.left, ctx->viewport.top - ctx->viewport.bottom};

    return v2d {(point.x / ctx->screen_size.x) * d.x + ctx->viewport.left, (point.y / ctx->screen_size.y) * d.y + ctx->viewport.bottom,};
}

uint8_t f(double x, double q, double p)
{
...
void iterate(context_t *ctx, pixel_data_t *pixel_data)
{
    ...
    if (std::abs(z) > 2)
    {
        ...
        pixel_data->color = Color {f(i, 1, 0), f(i, 1, 120), f(i, 1, 240), 255};
...
Rectangle fix_rect(Rectangle rect)
{
...
void worker(context_t *context, const int offset, const int width, const int screen_height)
{
...
void set_viewport(context_t *context, Rectangle rect)
{
    ...
    for (long double x = 0; x < context->screen_size.x; x += 1)
    {
        for (long double y = 0; y < context->screen_size.y; y += 1)
        {
            ...
            context->pixel_data[x][y] = pixel_data_t {{ 0, 0 }, /* z*/ { point.x, point.y }, /* c*/ 0, /* iteration*/ BLACK, /* color*/ false, /* done*/};
        }
    }
}
...
int main(void)
{
    ...
    context_t context = {{ screen_width, screen_height }, /* screen_size*/ /* Bottom and top are swapped for natural Y axis direction*/ { -2, 0.5, 1.12, -1.12 }, /* viewport*/ 100, /* max_iterations*/ std::vector(screen_width, std::vector<pixel_data_t>(screen_height)),};
    ...
    while (!WindowShouldClose())
    {
        ...
        sprintf(title, "Creative Coding: Mandelbrot Set [fps = %f]", 1 / deltatime);
        ...
        BeginDrawing();
        {
            ...
            {
                threads[i] = std::thread(&worker, &context, i * column_width, column_width, screen_height);
            }
