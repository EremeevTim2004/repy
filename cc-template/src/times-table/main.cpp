...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        BeginDrawing();
        {
            ...
            DrawRing(center, radius + 1, radius + 3, 0, 360, 200, line_color);

            for (int n = 0; n < lines_count; ++n)
            {
                Vector2 start = Vector2 {cosf(theta * n), sinf(theta * n)} * radius + center;

                Vector2 end = Vector2 {cosf(theta * multiple * n), sinf(theta * multiple * n)} * radius + center;
